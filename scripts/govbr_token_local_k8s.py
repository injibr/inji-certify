...
Usage:
    python3 scripts/govbr_token_homolog.py [--issuer ISSUER_ID]
...
DEFAULT_CERTIFY_URL = "http://localhost:30090/v1/certify"
DEFAULT_CERTIFY_IDENTIFIER = "http://inji-certify.inji-local.svc.cluster.local"

# Credential types to issue, one per issuer
CREDENTIALS = [
    {"issuer_id": "MDA",   "doc_type": "CAFCredential",     "format": "ldp_vc"},
    {"issuer_id": "INCRA", "doc_type": "CCIRCredential",    "format": "ldp_vc"},
    {"issuer_id": "MGI",   "doc_type": "ECACredential",     "format": "ldp_vc"},
    {"issuer_id": "MGI",   "doc_type": "br.gov.mgi.eca",   "format": "mso_mdoc"},
]
...
def generate_rsa_key_and_proof(certify_identifier, client_id, c_nonce=None):
    ...
    # JWT payload: matches mimoto's JoseUtil.generateJwt() implementation
    now = int(time.time())
    payload = {
        "iss": client_id,
        "sub": client_id,  # mimoto includes sub (same as iss)
        "aud": certify_identifier,
        "iat": now,
        "exp": now + 300,
    }
    # Include nonce if provided (mimoto extracts c_nonce from access_token)
    if c_nonce:
...
def generate_ec_key_and_proof(certify_identifier, client_id, c_nonce=None):
    """
    Generate an ephemeral EC P-256 key pair and build a signed OID4VCI proof JWT (ES256).
    Required for mso_mdoc — ISO 18013-5 only supports EC keys for deviceKey.
    """
    ...
    jwk = {
        "kty": "EC",
        "crv": "P-256",
        "x": b64url(pub_numbers.x.to_bytes(32, byteorder="big")),
        "y": b64url(pub_numbers.y.to_bytes(32, byteorder="big")),
    }
...
def make_proof_jwt(certify_identifier, client_id, c_nonce=None, fmt="ldp_vc"):
    """
    Try the cryptography library first, fall back to openssl CLI.
    Uses EC P-256 for mso_mdoc (ISO 18013-5 requirement), RSA for others.
    """
    try:
        from cryptography.hazmat.primitives.asymmetric import ec  # noqa: F401
        if fmt == "mso_mdoc":
            return generate_ec_key_and_proof(certify_identifier, client_id, c_nonce)
...
def request_credential(certify_url, access_token, proof_jwt, doc_type, issuer_id, fmt="ldp_vc"):
...
def load_token_cache():
    """Load the entire token cache from disk."""
    try:
...
def save_token_cache(cache):
    """Save the entire token cache to disk."""
    with open(TOKEN_CACHE_FILE, "w") as f:
        json.dump(cache, f, indent=2)


def get_cached_token(cache, issuer_id):
    """Return cached access_token for issuer if valid, else None."""
...
def cache_token(cache, issuer_id, access_token):
    """Cache an access token for a specific issuer."""
...
def main():
    parser = argparse.ArgumentParser(description="Issue credentials via Gov.br SSO + Inji Certify")
    parser.add_argument(
        "--issuer", "-i",
        action="append",
        dest="issuers",
    ...
    parser.add_argument(
        ...
        action="append",
        dest="formats",
        choices=["ldp_vc", "mso_mdoc", "vc+sd-jwt"],
        help="Only issue credential for specified format (can repeat)",
    )
    ...
    # Filter credentials if --issuer or --format specified
    credentials = CREDENTIALS
    if args.issuers:
        credentials = [c for c in credentials if c["issuer_id"] in args.issuers]
    if args.formats:
        credentials = [c for c in credentials if c["format"] in args.formats]
    if not credentials:
        print(f"ERROR: No matching credentials for issuers={args.issuers} formats={args.formats}")
        sys.exit(1)

    print("=" * 60)
    print("INJI CERTIFY CREDENTIAL ISSUANCE (Local K8s)")
    print("=" * 60)
    print(f"Environment:")
    ...
    for cred in credentials:
        ...
        # Get proof client_id from token audience
        proof_client_id = at_claims.get("aud", "")

        # Extract c_nonce from access_token (mimoto extracts this for replay protection)
        c_nonce = at_claims.get("c_nonce")

        # Generate proof JWT and request credential
        alg = "ES256" if fmt == "mso_mdoc" else "RS256"
        print(f"  Generating wallet proof JWT (alg={alg})...")
        if c_nonce:
            print(f"    Including c_nonce in proof JWT")
        proof_jwt = make_proof_jwt(certify_identifier, proof_client_id, c_nonce, fmt)

        print(f"  Requesting credential (format={fmt})...")
        result, status = request_credential(
            certify_url, access_token, proof_jwt, doc_type, issuer_id, fmt
        )
        results[f"{issuer_id}:{doc_type}"] = {"status": status, "response": result, "doc_type": doc_type, "format": fmt}

        print(f"  HTTP Status: {status}")
        if "credential" in result:
            print(f"  Result: ISSUED ({fmt})")
            print(f"\n  Credential:")
    ...
    for key, info in results.items():
        ...
        if status_str == "FAILED":
            all_ok = False
        print(f"  {key:30s} {info['format']:10s} HTTP {info['status']}  {status_str}")
