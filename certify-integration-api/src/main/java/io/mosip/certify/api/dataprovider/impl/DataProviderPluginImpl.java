...
public class DataProviderPluginImpl implements DataProviderPlugin {
    ...
    public JSONObject fetchData(Map<String, Object> identityDetails) throws DataProviderExchangeException {
        // INJIBR-CUSTOM: docType pode ser credentialConfigKeyId (ex: ECACredential, ECACredential-mdoc)
        // normaliza removendo sufixos de formato para encontrar o provider correto
        String docType = (String) identityDetails.get("docType");
        DataProviderService dataProviderService = instanceMap.get(docType);
        if (dataProviderService == null && docType != null) {
            // tenta sem sufixo de formato (ex: "ECACredential-mdoc" → "ECACredential")
            String normalized = docType.replaceAll("-(mdoc|sd-jwt|ldp)$", "");
            dataProviderService = instanceMap.get(normalized);
        }
